...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      // ─── MENÚ DE AYUDA (-list) ──────────────────────────────────────────────
      if (args[0] === '-list') {
        let helpText = `🦋 *NINO NAKANO PREMIUM* 🦋\n\n` +
      ...
      let pack = marca[0] || PACK_NAME;
      let author = marca.length > 1 ? marca[1] : PACK_AUTHOR;

      // ─── REINTEGRACIÓN DEL CANAL/NEWSLETTER ────────────────────────────────
      ...
      // ─── FFmpeg CON PARÁMETROS ANTIFANTASMA ───────────────────────────────
      // Forzamos yuva420p y q:v bajo para asegurar compatibilidad y bajo peso
      ...
      const ffmpegArgs = [
        ...
        '-lossless', '0',
        '-compression_level', '6',
        '-q:v', isVideo ? '30' : '50', // Calidad baja para stickers animados = no fantasma
        '-loop', '0',
        '-preset', 'picture',
        '-an', '-vsync', '0',
        outputPath
      ...
      // ─── INYECTOR DE EXIF (SIN DEPENDENCIAS EXTERNAS) ──────────────────────
      const finalSticker = await addExif(stickerBuffer, pack, author);

      // ─── ENVÍO FINAL CON EL CANAL/CONTEXTO ─────────────────────────────────
...
// ─── FUNCIONES TÉCNICAS (NO TOCAR) ───────────────────────────────────────────

async function addExif(buffer, pack, auth) {
    const json = { "sticker-pack-id": `nino-${Date.now()}`, "sticker-pack-name": pack, "sticker-pack-publisher": auth, "emojis": ["🦋"] };
...
const buildFFmpegFilters = (args) => {
  ...
  const effectArgs = { '-blur': 'gblur=sigma=5', '-sepia': 'colorchannelmixer=.393:.769:.189:0:.349:.686:.168:0:.272:.534:.131', '-invert': 'negate', '-grayscale': 'hue=s=0', '-flip': 'hflip', '-flop': 'vflip' };
  
  let filters = [`scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=0x00000000`, `format=rgba`];
  
  args.forEach(arg => {
    if (effectArgs[arg]) filters.push(effectArgs[arg]);
    if (shapeArgs[arg]) {
      const shape = shapeArgs[arg];
      if (shape === 'circle') filters.push(`geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='if(lte((X-256)*(X-256)+(Y-256)*(Y-256),256*256),255,0)'`);
      if (shape === 'heart') filters.push(`geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='if(lte(pow((X-256)/160,2)+pow((Y-256)/160,2)-1,3)-pow((X-256)/160,2)*pow((Y-256)/160,3),0),255,0)'`);
    }
  });

  filters.push('format=yuva420p'); // CRÍTICO: Evita el sticker fantasma
  return filters.join(',');
};
